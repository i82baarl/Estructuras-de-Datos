...
Response PacketProcessor::process(const Packet &packet)
{
    ...
    if(!planificador_.is_empty()){

        if(packet.arrival_time >= planificador_.front()){

            planificador_.deque();
        }
    }

    if(planificador_.size() >= size_){
    ...
    if(planificador_.is_empty()){
        ...
        time = planificador_.back();
    }

    planificador_.enque(time + packet.process_time);
